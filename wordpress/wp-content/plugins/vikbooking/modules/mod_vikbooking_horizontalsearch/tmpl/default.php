...
<div class="vbmodhorsearchmaindiv <?php echo $params->get('datestype') == 'human' ? 'vbo-horizsearch-humanformat-wrap' : 'vbo-horizsearch-standardformat-wrap'; ?>">
	<form action="<?php echo JRoute::rewrite('index.php?option=com_vikbooking&task=search&Itemid=' . $params->get('itemid', 0)); ?>" method="post">
...
'.($params->get('datestype') == 'human' ? VikBookingWidgetHorizontalSearch::getMonWdayScript($params->get('mondayslen'), $randid) : '').'
...
jQuery(function() {
	...
	jQuery('#checkindatemod".$randid."').datepicker({
		".($params->get('datestype') != 'human' ? "showOn: 'focus'," : '')."
		".($params->get('datestype') == 'human' ? "altField: '#inp-checkindatemod{$randid}',\n" : '')."
		...
		onSelect: function( selectedDate ) {
			...
			".($params->get('datestype') == 'human' ? "vbFormatCalDateMod".$randid."('from');" : '')."
		}
	});
	...
	jQuery('#checkoutdatemod".$randid."').datepicker({
		".($params->get('datestype') != 'human' ? "showOn: 'focus'," : '')."
		".($params->get('datestype') == 'human' ? "altField: '#inp-checkoutdatemod{$randid}',\n" : '')."
		numberOfMonths: ".($is_mobile ? '1' : '2').",".(count($wdaysrestrictions) > 0 || count($wdaysrestrictionsrange) > 0 ? "\nbeforeShowDay: vbIsDayDisabledCheckoutmod".$randid.",\n" : "\nbeforeShowDay: vbCheckClosingDatesOutmod".$randid.",\n")."
		onSelect: function( selectedDate ) {
			vbCalcNightsMod".$randid."();
			".($params->get('datestype') == 'human' ? "vbFormatCalDateMod".$randid."('to');" : '')."
		}
	});
...
// number of nights calculated (displayed above for human readable layout)
if ($params->get('datestype') != 'human') {
	?>
		<div class="vbmodhorsearchtotnights" id="vbjstotnightsmod<?php echo $randid; ?>"></div>
	<?php
